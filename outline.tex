...
%lst js support, delete if not needed
...
\noindent
\textbf{專案連結：} \\
\url{https://youtu.be/VgkbBeDXqa0} \quad (Demo 影片) \\
\url{https://github.com/Phanatic34/114-1_database_final_project.git} \quad (GitHub 倉庫)
...
\begin{enumerate}
    ...
    \item \textbf{軟刪除機制：}當使用者刪除帳號時，系統採用軟刪除機制，設置 Deleted\_at 時間戳，保留所有歷史資料（交易紀錄、評價、訊息等）。已刪除帳號無法再登入，其商品會自動下架，其他用戶無法對其商品發起新請求或發送訊息。
    
    \item \textbf{搜尋紀錄：}系統會自動將所有商品搜尋行為記錄到 MongoDB 的 search\_logs collection，包括用戶 ID（如果已登入）、搜尋關鍵字、篩選條件、結果數量與時間戳，用於分析使用者搜尋行為與推薦系統。
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{帳號管理：}使用者可更新個人資料、修改密碼或刪除帳號（軟刪除機制，保留歷史資料）。
\end{enumerate}
...
TRADE\_REQUEST 為交易請求的關聯實體，用以記錄使用者之間的互動。當某位使用者想購買或交換他人商品時，系統會產生一筆交易請求紀錄，其中包含請求者（Requester\_id）、目標商品（Target\_product\_id）及請求類型（Request\_type）等欄位。若為交換請求，還會紀錄該使用者提供的交換商品（Offered\_product\_id）。請求狀態會依交易流程變化，從 Pending、Accepted、Completed 至 Cancelled。當請求被接受後，買賣雙方可以透過 Buyer\_confirmed\_handoff 和 Seller\_confirmed\_handoff 欄位確認已面交，當雙方都確認後，系統會自動建立 TRANSACTION 記錄並完成交易。

TRANSACTION 則對應於實際完成的交易紀錄。當請求被接受（Status = 'Accepted'）且買賣雙方都確認已面交（Buyer\_confirmed\_handoff = TRUE 且 Seller\_confirmed\_handoff = TRUE）後，系統會自動產生交易紀錄，其中包含完成日期、交易金額與付款狀態等欄位。此關聯可視為 TRADE\_REQUEST 的延伸，兩者呈現一對一關係。交易完成後，買賣雙方可以選擇對對方進行評價（點讚、倒讚或不做評價）。

TRADE\_WISH 表示使用者對尚未出現的商品提出的開放式交換需求。使用者可描述欲交換之物品名稱、類別與條件，系統會將該許願資訊公開，並在有相符商品上架時提供通知。此設計使交換流程不僅限於即時配對，也能支持延遲撮合的情境。

REVIEW 為交易評價實體，用於記錄使用者對已完成交易的評價與評論。當交易完成後，買賣雙方皆可選擇對該筆交易進行評價（可選功能）。評價機制支援「點讚」（Rating = 5）、「倒讚」（Rating = 1）或「不做評價」（不創建 REVIEW 記錄）。好評率計算公式為「按讚數 / (按讚數 + 倒讚數) * 100\%」，不做評價的交易不計入好評率。每筆交易可包含買方對賣方的評價，以及賣方對買方的評價，形成雙向評價系統。
...
Password       & 密碼           & varchar(255)&     & Not Null         & \Note{使用 bcrypt hash，長度增加} \\
...
Created\_at    & 建立時間       & timestamp   &     & Default Current\_Timestamp &  \\
Updated\_at    & 更新時間       & timestamp   &     & Default Current\_Timestamp & \Note{由觸發器自動更新} \\
Deleted\_at    & 刪除時間       & timestamp   &     & Nullable         & \Note{軟刪除標記，NULL 表示未刪除} \\
\bottomrule
...
Description       & 商品描述         & TEXT         &     &                  & \Note{改用 TEXT 支援較長描述} \\
...
Image\_url        & 圖片網址         & TEXT         &     &                  & \Note{改用 TEXT 支援 base64 圖片} \\
Post\_date        & 上架日期         & timestamp    &     & Not Null         &  \\
Created\_at       & 建立時間         & timestamp    &     & Default Current\_Timestamp &  \\
Updated\_at       & 更新時間         & timestamp    &     & Default Current\_Timestamp & \Note{由觸發器自動更新} \\
\midrule
...
Buyer\_confirmed\_handoff & 買家確認面交 & boolean     &     & Default false    & \Note{買家確認已面交} \\
Seller\_confirmed\_handoff & 賣家確認面交 & boolean     &     & Default false    & \Note{賣家確認已面交} \\
Created\_at          & 建立時間            & datetime    &     & Not Null         &  \\
Updated\_at          & 更新時間            & datetime    &     & Not Null         &  \\
\midrule
\multicolumn{4}{l}{\textbf{Referential triggers}} & \textbf{On Delete} & \textbf{On Update} \\
...
Rating          & 評分             & int         &     & Not Null         & \Note{1 = 倒讚，5 = 點讚；評價為可選，不做評價時不創建記錄} \\
Comment         & 評論內容         & varchar(500)&     & Nullable         &  \\
Created\_at     & 建立時間         & timestamp   &     & Not Null         &  \\
\midrule
\multicolumn{4}{l}{\textbf{Unique Constraint}} & \textbf{Constraint Name} & \textbf{Columns} \\
\multicolumn{4}{l}{unique\_review\_per\_transaction} & Unique & (Transaction\_id, Reviewer\_id, Reviewee\_id) \\
\midrule
\multicolumn{4}{l}{\textbf{Referential triggers}} & \textbf{On Delete} & \textbf{On Update} \\
...
Created\_at          & 建立時間         & timestamp   &     & Not Null         &  \\
Resolved\_at         & 處理完成時間     & timestamp   &     & Nullable         &  \\
\midrule
\multicolumn{4}{l}{\textbf{Check Constraint}} & \textbf{Constraint Name} & \textbf{Condition} \\
\multicolumn{4}{l}{check\_report\_target} & Check & Reported\_product\_id Is Not Null Or Reported\_user\_id Is Not Null \\
\midrule
\multicolumn{4}{l}{\textbf{Referential triggers}} & \textbf{On Delete} & \textbf{On Update} \\
...
Listing 3: 接受交易請求 SQL 指令


\subsubsection{確認已面交（Confirm Handoff）}

若要實現此功能，假設情境為「買家或賣家確認已面交，當雙方都確認後，系統自動建立交易紀錄並完成交易。」則對應的 SQL 指令如下，系統會更新 TRADE\_REQUEST 資料表中對應的確認狀態，當雙方都確認後，自動建立 TRANSACTION 記錄。
...
Where Request_id = 3001 And Target_product_id IN 
    (Select Product_id From PRODUCT Where Owner_id = 1001);

-- 當雙方都確認後，自動建立交易紀錄（由系統觸發）
Begin Transaction;
...
Listing 5: 查詢使用者提出的交易請求 SQL 指令

\subsubsection{完成交易（Complete Transaction）}

若要實現此功能，假設情境為「當交易請求 Request\_id '3001' 被接受後，買賣雙方都確認已面交（Buyer\_confirmed\_handoff = TRUE 且 Seller\_confirmed\_handoff = TRUE），系統建立一筆交易紀錄，交易金額 Total\_price 為 '280'，完成日期 Complete\_date 為 '2024-11-05 16:00:00'。」則對應的 SQL 指令如下，系統會在 TRANSACTION 資料表中新增一筆交易紀錄，並更新對應的 TRADE\_REQUEST 狀態為 'Completed'，同時更新商品狀態為 'sold' 或 'exchanged'。

\textbf{交易完成流程：}
\begin{enumerate}
    \item 賣家接受交易請求（Status = 'Accepted'）
    \item 買賣雙方確認已面交（Buyer\_confirmed\_handoff = TRUE 且 Seller\_confirmed\_handoff = TRUE）
    \item 系統自動建立 TRANSACTION 記錄
    \item 更新 TRADE\_REQUEST 狀態為 'Completed'
    \item 更新商品狀態為 'sold'（購買）或 'exchanged'（交換）
\end{enumerate}
...
Set Status = 'exchanged',
    Updated_at = CURRENT_TIMESTAMP
Where Product_id = (Select Offered_product_id 
                    From TRADE_REQUEST 
                    Where Request_id = 3001
...
Listing 7: 查詢交易紀錄 SQL 指令

\subsubsection{新增評價（Add Review）}

若要實現此功能，假設情境為「使用者代號 Reviewer\_id '2001' 對交易代號 Transaction\_id '4001' 進行評價，被評價者代號 Reviewee\_id 為 '1001'，評分 Rating 為 '5'（點讚），評論 Comment 為 '交易順利，商品狀況良好'，建立時間 Created\_at 為 '2024-11-06 10:00:00'。」則對應的 SQL 指令如下，系統會在 REVIEW 資料表中新增一筆評價紀錄。

\textbf{評價機制說明：}
\begin{itemize}
    \item 評價為可選功能，買賣雙方都可以選擇對對方進行評價
    \item 評分只能選擇 1（倒讚）或 5（點讚）
    \item 如果選擇不做評價，則不創建 REVIEW 記錄
    \item 好評率計算公式：\texttt{(按讚數 / (按讚數 + 倒讚數)) * 100\%}，不做評價的交易不計入好評率
\end{itemize}
...
-- 不做評價：不執行任何 INSERT 操作
\end{lstlisting}

Listing 8: 新增評價 SQL 指令
...
Listing 11: 提出檢舉 SQL 指令

\subsubsection{查詢商品（Query Products by Category）}

若要實現此功能，假設情境為「某使用者想透過兩個條件，商品分類 Category\_name 為 'Textbooks' 或商品名稱 Product\_name 內有 '微積分' 來查詢商品。」則對應的 SQL 指令如下，系統會執行該查詢，並回傳符合這些條件的商品資訊。系統會自動過濾已刪除用戶的商品，並將搜尋行為記錄到 MongoDB。
...
Where (c.Category_name = 'Textbooks'
  ...
  And u.Deleted_at IS NULL;  -- 過濾已刪除用戶的商品
\end{lstlisting}

Listing 12: 查詢商品 SQL 指令

\textbf{搜尋紀錄：}當使用者執行搜尋時，系統會自動將搜尋行為記錄到 MongoDB 的 search\_logs collection，包括用戶 ID（如果已登入）、搜尋關鍵字、篩選條件、結果數量與時間戳。
...
Listing 22: 建立評價被評價者索引

\subsection{其他效能優化索引}

除了上述核心索引外，本系統還建立了以下索引以進一步優化查詢效能：

\paragraph{PRODUCT 表額外索引}
\begin{lstlisting}[language=SQL]
-- 優化查詢特定使用者的商品
...
Listing 23: 建立商品擁有者及分類索引

\paragraph{TRADE\_REQUEST 表額外索引}
\begin{lstlisting}[language=SQL]
-- 優化查詢使用者提出的請求
Create Index idx_request_requester On TRADE_REQUEST (Requester_id);

-- 優化查詢特定商品的請求
Create Index idx_request_target_product On TRADE_REQUEST (Target_product_id);
\end{lstlisting}

Listing 24: 建立交易請求額外索引

\paragraph{MESSAGE 表額外索引}
\begin{lstlisting}[language=SQL]
-- 優化查詢發送者的訊息
...
\paragraph{REVIEW 表額外索引}
\begin{lstlisting}[language=SQL]
-- 優化查詢特定交易的評價
Create Index idx_review_transaction On REVIEW (Transaction_id);
\end{lstlisting}

Listing 26: 建立評價交易索引

\paragraph{TRANSACTION 表索引}
\begin{lstlisting}[language=SQL]
-- 優化查詢特定請求的交易紀錄
Create Index idx_transaction_request On TRANSACTION (Request_id);

-- 優化查詢特定商品的交易紀錄
Create Index idx_transaction_target_product On TRANSACTION (Target_product_id);
\end{lstlisting}
...
\paragraph{USER 表索引}
\begin{lstlisting}[language=SQL]
-- 優化登入查詢（Email 已為 UNIQUE，自動建立索引）
-- 優化學號查詢（Student_id 已為 UNIQUE，自動建立索引）
-- 額外建立索引以優化其他查詢場景
...
\subsection{自動更新時間戳記觸發器}

為確保資料表更新時間（Updated\_at）欄位能自動維護，本系統建立了觸發器函數與對應觸發器：
...
-- 在 TRADE_REQUEST 表建立觸發器
Create Trigger update_trade_request_updated_at 
    Before Update On TRADE_REQUEST
    For Each Row Execute Function update_updated_at_column();

-- 在 TRADE_WISH 表建立觸發器
Create Trigger update_trade_wish_updated_at 
    Before Update On TRADE_WISH
    For Each Row Execute Function update_updated_at_column();
\end{lstlisting}

Listing 29: 建立自動更新時間戳記觸發器

\subsection{資料完整性約束}

為確保資料完整性，本系統建立了以下額外約束：

\paragraph{REVIEW 表唯一性約束}
為防止同一筆交易中，同一評價者對同一被評價者重複評價，建立唯一性約束：

\begin{lstlisting}[language=SQL]
Alter Table REVIEW
Add Constraint unique_review_per_transaction 
    Unique (Transaction_id, Reviewer_id, Reviewee_id);
\end{lstlisting}

Listing 30: 建立評價唯一性約束

\paragraph{REPORT 表檢查約束}
為確保檢舉必須針對商品或使用者其中一項，建立檢查約束：
...
Listing 31: 建立檢舉目標檢查約束

\subsection{資料類型優化}

為支援較長的商品描述與 base64 編碼的圖片網址，本系統將以下欄位從 VARCHAR 改為 TEXT 類型：

\begin{lstlisting}[language=SQL]
-- 修改 PRODUCT 表的 Description 欄位
Alter Table PRODUCT Alter Column Description Type TEXT;

-- 修改 PRODUCT 表的 Image_url 欄位
Alter Table PRODUCT Alter Column Image_url Type TEXT;
\end{lstlisting}

Listing 32: 優化商品描述與圖片欄位類型

\subsection{軟刪除功能實作}

本系統實作軟刪除機制，當使用者刪除帳號時，系統不會真正刪除資料，而是設置 Deleted\_at 時間戳，保留所有歷史資料。

\paragraph{添加 Deleted\_at 欄位}
\begin{lstlisting}[language=SQL]
...
WHERE owner_id = 1001 AND status IN ('available', 'reserved');

COMMIT;
\end{lstlisting}

Listing 34: 軟刪除帳號 SQL 指令

\paragraph{查詢時過濾已刪除帳號}
\begin{lstlisting}[language=SQL]
-- 查詢商品時過濾已刪除用戶的商品
...
WHERE p.status = 'available' AND u.deleted_at IS NULL;
...
Listing 35: 過濾已刪除帳號 SQL 指令

\subsection{MongoDB 搜尋紀錄功能}

本系統使用 MongoDB 記錄所有商品搜尋行為，用於分析使用者搜尋模式與推薦系統。

\paragraph{搜尋紀錄 Collection 結構}
MongoDB 的 search\_logs collection 包含以下欄位：
\begin{itemize}
    \item \texttt{user\_id}: 使用者代號（如果已登入，否則為 NULL）
    \item \texttt{search\_keywords}: 搜尋關鍵字
    \item \texttt{filters}: 篩選條件（JSON 格式，包含 category\_id、trade\_option、status 等）
    \item \texttt{result\_count}: 搜尋結果數量
    \item \texttt{timestamp}: 搜尋時間戳
\end{itemize}
...
db.search_logs.createIndex({ "search_keywords": "text" });  // 全文搜尋索引
\end{lstlisting}

Listing 36: MongoDB 搜尋紀錄索引

\paragraph{記錄搜尋行為}
當使用者搜尋商品時，系統會自動記錄到 MongoDB：
...
SearchLog.log_search(
    user_id=1001,
    keywords="iPhone",
    filters={"category_id": "1", "trade_option": "sale"},
    result_count=5
)
...
在使用者點擊「送出交易」按鈕\emph{之後}取得鎖定。此時，系統會執行\emph{第二次檢查}（重新查詢），以確認該商品的目標請求狀態是否仍然可用（即尚未被其他使用者提出請求）。若確認可用，則將新的請求記錄新增至 TRADE\_REQUEST 表，然後釋放鎖定。若發現不可用（已被請求），則不新增記錄，釋放鎖定，並在畫面上顯示錯誤訊息給使用者。此方法也能有效防止重複請求。


\subsubsection{實作細節}

此功能使用 Python 的 \texttt{threading} 套件與 PostgreSQL 的 \texttt{SELECT FOR UPDATE} 語句實作。具體而言，在 \texttt{./api/routes/trade\_requests.py} 中的 \texttt{create\_trade\_request} 函數內：

\begin{enumerate}
    \item 使用 \texttt{threading.Lock} 的 \texttt{acquire()} 方法取得應用層鎖定。
    \item 使用 \texttt{SELECT FOR UPDATE} 鎖定商品行，防止其他交易同時修改該商品狀態。
    \item 檢查該商品是否已有待處理的請求（Status IN ('Pending', 'Accepted')），使用 \texttt{SELECT FOR UPDATE} 鎖定相關請求行。
    \item 再次檢查商品狀態（使用 \texttt{SELECT FOR UPDATE}），確保商品仍為 'available'。
    \item 若該商品\emph{未被請求且狀態為 'available'}，則：
    \begin{itemize}
        \item 將新的請求記錄新增至 TRADE\_REQUEST 表
        \item 立即將目標商品狀態更新為 'reserved'，防止其他使用者同時提出請求
        \item 如果是交換請求，也將交換商品狀態更新為 'reserved'
    \end{itemize}
    \item 使用 \texttt{lock.release()} 釋放應用層鎖定，允許其他連線使用此函數。
    \item 若該商品\emph{已被請求或狀態不可用}，則釋放鎖定，並回傳「請求失敗」訊息。
\end{enumerate}
...
此實作結合了應用層鎖定（threading.Lock）與資料庫層鎖定（SELECT FOR UPDATE），確保在併發環境下，同一商品不會被多個使用者同時成功提出請求，從而維護了資料的一致性和系統的正確性。
...
本次專題開發，讓我深刻體會到資料庫設計與系統整合的複雜性。從 ER Diagram 設計、正規化分析到實際實作，每一步都需要仔細思考資料關聯性與完整性。過程中遇到許多挑戰，如併行控制、軟刪除機制、前後端整合等問題，透過不斷除錯與優化，最後終於完成了一個功能完整的交易平台。這次經驗讓我對關聯式資料庫與 NoSQL 資料庫的應用有了更深入的理解。
\setlength{\parindent}{0pt} % 取消首行縮排

周建凱：

在這次專題中，我主要負責的任務是資料庫設計、前端實作以及簡報製作。因為同時接觸到前後端，讓我更完整地看見一個系統從底層資料結構到實際畫面呈現的整體流程。

在資料庫部分，最大的挑戰在於釐清各個實體與關係，並透過正規化降低重複與異常，否則在實作功能時很容易被不良的 schema 牽制。前端開發則逼迫我把抽象的交易流程轉換成具體又直覺的操作介面，同時必須仔細思考按鈕位置、狀態切換與錯誤訊息要如何設計，才能讓使用者知道「現在發生什麼事」與「下一步可以做什麼」。

最後，我很感謝組員在整個專案實作過程的積極參與、討論及建議，無論是在技術交流還是合作溝通上，我都從這次專題中獲得了很多成長與收穫。\\
\setlength{\parindent}{0pt} % 取消首行縮排

宋承陽：之前在資訊管理導論的時候，完全不知道自己期末的資料庫到底做了甚麼，這次機會讓我對於整體資料庫架構的設計有更進一步的認識。這次專案主要使用關聯式資料庫，因此如何一開始確保大架構的穩定相當重要。要在實作前仔細推敲使用者行為與預期系統回饋頗有挑戰性。而關於書面報告，還好前面幾次作業已經被 tikz 折磨過了，這次 ER Diagram 畫起來雖然還是花了不少時間，但整體而言順暢許多。
