...
\subsubsection{計算好評率（Calculate Positive Rate）}

若要實現此功能，假設情境為「計算使用者代號 User\_id '1001' 的好評率。」則對應的 SQL 指令如下，系統會計算該使用者的好評率（按讚數 / (按讚數 + 倒讚數) * 100\%），不做評價的交易不計入。
...
SELECT 
    ...
    CASE 
        WHEN COUNT(CASE WHEN rating IN (1, 5) THEN 1 END) > 0 
        THEN ROUND(
            COUNT(CASE WHEN rating = 5 THEN 1 END)::NUMERIC / 
            COUNT(CASE WHEN rating IN (1, 5) THEN 1 END)::NUMERIC * 100, 
            1
        )
        ELSE 0 
    END as positive_rate
FROM review
WHERE reviewee_id = 1001;
\end{lstlisting}

Listing 11a: 計算好評率 SQL 指令
